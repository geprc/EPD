// class GxGDEP015OC1 : Display class for GDEP015OC1 e-Paper from Dalian Good
// Display Co., Ltd.: www.good-display.com
//
// based on Demo Example from Good Display, available here:
// http://www.good-display.com/download_detail/downloadsId=515.html Controller :
// IL3829 : http://www.good-display.com/download_detail/downloadsId=534.html
//
// Author : J-M Zingg
//
...
const uint8_t GxGDEP015OC1::GDOControl[] = {
    0x01, (GxGDEP015OC1_Y_PIXELS - 1) % 256, (GxGDEP015OC1_Y_PIXELS - 1) / 256,
    0x00};  // for 1.54inch
const uint8_t GxGDEP015OC1::softstart[] = {0x0c, 0xd7, 0xd6, 0x9d};
const uint8_t GxGDEP015OC1::VCOMVol[] = {0x2c, 0x9b};  // VCOM 7c
const uint8_t GxGDEP015OC1::DummyLine[] = {0x3a,
                                           0x1a};       // 4 dummy line per gate
...
void GxGDEP015OC1::drawBitmap(const uint8_t* bitmap, uint16_t x, uint16_t y,
...
void GxGDEP015OC1::drawBitmap(const uint8_t* bitmap, uint32_t size,
                              int16_t mode) {
    if (_current_page != -1) return;
    // example bitmaps are made for y-decrement, x-increment, for origin on
    // opposite corner bm_flip_x for normal display (bm_flip_y would be rotated)
...
void GxGDEP015OC1::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
    ...
    uint16_t xe_d8 = xe / 8;
    _Init_Part(0x03);
    _SetRamArea(xs_d8, xe_d8, y % 256, y / 256, ye % 256,
                ye / 256);                    // X-source area,Y-gate area
...
void GxGDEP015OC1::_writeToWindow(uint16_t xs, uint16_t ys, uint16_t xd,
                                  uint16_t yd, uint16_t w, uint16_t h) {
    ...
    uint16_t yse = ys + h - 1;
    _SetRamArea(xds_d8, xde_d8, yd % 256, yd / 256, yde % 256,
                yde / 256);                      // X-source area,Y-gate area
...
void GxGDEP015OC1::_writeData(uint8_t data) { IO.writeDataTransaction(data); }

void GxGDEP015OC1::_writeCommandData(const uint8_t* pCommandData,
                                     uint8_t datalen) {
    if ((_busy >= 0) && digitalRead(_busy)) {
...
void GxGDEP015OC1::_setRamDataEntryMode(uint8_t em) {
    ...
    switch (em) {
        case 0x00:  // x decrease, y decrease
            _SetRamArea(xPixelsPar / 8, 0x00, yPixelsPar % 256,
                        yPixelsPar / 256, 0x00,
                        0x00);  // X-source area,Y-gate area
            _SetRamPointer(xPixelsPar / 8, yPixelsPar % 256,
                           yPixelsPar / 256);  // set ram
            break;
        case 0x01:  // x increase, y decrease : as in demo code
            _SetRamArea(0x00, xPixelsPar / 8, yPixelsPar % 256,
                        yPixelsPar / 256, 0x00,
                        0x00);  // X-source area,Y-gate area
            _SetRamPointer(0x00, yPixelsPar % 256,
                           yPixelsPar / 256);  // set ram
            break;
        case 0x02:  // x decrease, y increase
            _SetRamArea(xPixelsPar / 8, 0x00, 0x00, 0x00, yPixelsPar % 256,
                        yPixelsPar / 256);  // X-source area,Y-gate area
            _SetRamPointer(xPixelsPar / 8, 0x00, 0x00);  // set ram
            break;
        case 0x03:  // x increase, y increase : normal mode
            _SetRamArea(0x00, xPixelsPar / 8, 0x00, 0x00, yPixelsPar % 256,
                        yPixelsPar / 256);     // X-source area,Y-gate area
...
void GxGDEP015OC1::_SetRamArea(uint8_t Xstart, uint8_t Xend, uint8_t Ystart,
...
void GxGDEP015OC1::_SetRamPointer(uint8_t addrX, uint8_t addrY,
                                  uint8_t addrY1) {
    _writeCommand(0x4e);
    _writeData(addrX);
    _writeCommand(0x4f);
...
void GxGDEP015OC1::drawPaged(void (*drawCallback)(void)) {
    ...
    _Init_Full(0x03);
    _writeCommand(0x24);
    for (_current_page = 0; _current_page < GxGDEP015OC1_PAGES;
         _current_page++) {
        fillScreen(GxEPD_WHITE);
        drawCallback();
...
void GxGDEP015OC1::drawPaged(void (*drawCallback)(const void*, const void*),
...
void GxGDEP015OC1::drawPagedToWindow(void (*drawCallback)(void), uint16_t x,
    ...
    for (_current_page = 0; _current_page < GxGDEP015OC1_PAGES;
         _current_page++) {
        uint16_t yds =
            gx_uint16_max(y, _current_page * GxGDEP015OC1_PAGE_HEIGHT);
        uint16_t yde = gx_uint16_min(
            y + h, (_current_page + 1) * GxGDEP015OC1_PAGE_HEIGHT);
        if (yde > yds) {
            fillScreen(GxEPD_WHITE);
            drawCallback();
...
void GxGDEP015OC1::drawPagedToWindow(void (*drawCallback)(uint32_t), uint16_t x,
                                     uint16_t y, uint16_t w, uint16_t h,
                                     uint32_t p) {
    if (_current_page != -1) return;
    _rotate(x, y, w, h);
...
void GxGDEP015OC1::drawPagedToWindow(void (*drawCallback)(const void*),
                                     uint16_t x, uint16_t y, uint16_t w,
                                     uint16_t h, const void* p) {
    if (_current_page != -1) return;
    _rotate(x, y, w, h);
    if (!_using_partial_mode) {
        eraseDisplay(false);
...
void GxGDEP015OC1::drawPagedToWindow(
    void (*drawCallback)(const void*, const void*), uint16_t x, uint16_t y,
    uint16_t w, uint16_t h, const void* p1, const void* p2) {
    if (_current_page != -1) return;
    _rotate(x, y, w, h);
    if (!_using_partial_mode) {
        eraseDisplay(false);
...
void GxGDEP015OC1::drawCornerTest(uint8_t em) {
    ...
    for (uint32_t y = 0; y < GxGDEP015OC1_HEIGHT; y++) {
        for (uint32_t x = 0; x < GxGDEP015OC1_WIDTH / 8; x++) {
            ...
            if ((x > GxGDEP015OC1_WIDTH / 8 - 4) &&
                (y > GxGDEP015OC1_HEIGHT - 25))
                data = 0x00;
